...
const html = `
  ...
  <ul>
    <li>Card brand: <%= payment.card_brand %></li>
    <li>Card number: **** **** **** <%= payment.card_last4 %></li>
  </ul>
  ...
  <ul>
  <% _.forEach(games, function(game) { %>
    <li><a href="http://localhost:3000/game/<%= game.slug %>"><%= game.name %></a>
     - Price: <strong>$ <%= Number(game.price).toFixed(2) %></strong></li>
  <% }); %>
  </ul>
`;
